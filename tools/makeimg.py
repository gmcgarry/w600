...
def main(argv):
    ...
    upd_img_addr = struct.pack('<I', int(argv[6], 16) if int(argv[3]) != 2 else 0)
    upd_img_len = struct.pack('<I', bin_len if int(argv[3]) != 2 else 0)
    upd_checksum = struct.pack('<I', bin_crc if int(argv[3]) != 2 else 0)
